$(function() {
	...
	var NameWinnerView = Backbone.View.extend({
		...
		remove: function(){
			...
			$("#selecting-name").removeClass("final-winner")
								.html("<div id='pick-another'>Random pick again...</div>");

      if (!$("#selecting-name").hasClass('final-winner') && Entries.length == 0) {
	...
	var App = Backbone.View.extend({
		...
		createEntriesFromFile: function() {
			...
			$("#file-input").val("");
			$("#file-sample").text("");
			$("#file-area").attr("style",'display:none');
		},
      ...
      if (Entries.length>0) {
        var buttons = $('#buttons');
        var file_area = $('#file-area');
        if (!buttons.hasClass('hidden-on-pick') || !file_area.hasClass('hidden-on-pick')) {
          buttons.addClass('hidden-on-pick');
          file_area.addClass('hidden-on-pick');
        }
		...
		clearAllSafety: function() {
			this.$el.append("<div id='clear-all' class='alert alert-danger hidden-trigger'>CLICK HERE CLEAR ALL</div>");
